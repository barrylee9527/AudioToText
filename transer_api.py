...
class RequestApi(object):
    ...
    def get_result_request(self, taskid):
        return self.gene_request_result(api_get_result, data=self.gene_params(api_get_result, taskid=taskid))

    def gene_request_result(self, apiname, data, files=None, headers=None):
        response = requests.post(lfasr_host + apiname, data=data, files=files, headers=headers)
        result = response.json()
        if result["ok"] == 0:
            result_temp = re.findall(r'"onebest":"(.*?)","', result['data'][1:-1])
            res = ''
            for i in result_temp:
                res = res + i
            print("{} success:".format(apiname) + str(res))
            return res
        else:
            print("{} error:".format(apiname) + str(result))
            return '音频文件解析错误'
